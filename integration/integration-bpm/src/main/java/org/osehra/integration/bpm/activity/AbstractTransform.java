...
import org.osehra.integration.util.xsl.ResultType;
...
public abstract class AbstractTransform extends TransitionImpl {
	 ...
	 *
	 * @param theResultType
	 *            the result type XML or String.
	 * @uml.property name="resultType"
	 */
	public void setResultType(final ResultType theResultType) {
	 ...
	 */
	protected Object transform(final ProcessContext context,
		...
		try {
			...
			if (ResultType.STRING.equals(this.resultType)) {
				...
				//TODO: Change code to not escape the CDATA sections
				resultDoc = this.stringToXML.transform(stringWriter.toString());
				resultDoc = DOMHelper.cleaupDocument(resultDoc);
			} else { // Use DOMResult

			...
			<xsl:when test='n1:text/@representation="B64"'>
				<pre>
					...
					<!-- TODO: Asha Amritraj - Hardcoded C62 for now. Make it generic in
						next release -->
						<script type="text/javascript">
							function navigateUrl(mediaType) {
								var url = document.location.href;
								url = url.replace(".xml", "_attachment" + mediaType);
								location.href=url;
							}
						</script>
						<xsl:if test='n1:text/@mediaType="application/pdf"'>
							<a href="javascript:navigateUrl('.pdf');">View PDF Document</a>
						</xsl:if>
						<xsl:if test='n1:text/@mediaType="image/jpeg"'>
							<a href="javascript:navigateUrl('.jpg');">View Image Attachment</a>
						</xsl:if>
						<xsl:if test='n1:text/@mediaType="text/plain"'>
							<a href="javascript:navigateUrl('.txt');">View Text Document</a>
						</xsl:if>
				</pre>
			</xsl:when>
